...
import { create_review } from "../api/productos";
...
const Reviews = ({productId, reviews}: Props) => {
    ...
    const createReview = useMutation({
        mutationFn: () => create_review(descripcion, valoracion, productId),
        onSuccess: () => {
                        ...
                        {[...Array(5)].map((_, index) => {
                          ...
                          return (
                            <button
                              type="button"
                              key={index}
                              className={index <= ((valoracion && hover) || hover) ? "on" : "off"}
                              onClick={() => setValoracion(index)}
                              onMouseEnter={() => setHover(index)}
                              onMouseLeave={() => setHover(valoracion)}
                ...
                <div className="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-6">
                    <div className="mx-auto max-w-screen-sm">
                        <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">
                            Reviews De Las Camaras
                        </h2>
                        <p className="mb-8 font-light text-gray-500 lg:mb-16 sm:text-xl dark:text-gray-400">
                            Explore las opiniones sobre este producto
                        </p>
                        <button
                        onClick={() => {
                            setShow(true)
